...
- (void)prepareLayout{
    ...
    //保存内容尺寸
    self.contentSize = CGSizeMake(self.contentInset.left + self.contentInset.right + maxNumberOfItems * (self.itemSize.width + self.interitemSpacing) + self.interitemSpacing, self.contentInset.top + self.contentInset.bottom + numberOfSections * (self.itemSize.height + self.lineSpacing) + self.lineSpacing);
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    attributes.frame = CGRectMake(
                                  self.contentInset.left + (self.itemSize.width + self.interitemSpacing) * indexPath.row + self.interitemSpacing,
